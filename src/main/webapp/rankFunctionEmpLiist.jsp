...
<%
	...
	//테이블 쿼리 작성
	/*
	select 번호, 전체순위, 사원ID, 마지막이름, 급여 from(select rownum 번호, 전체순위, 사원ID, 마지막이름, 급여 from(select employee_id 사원ID, last_name 마지막이름, salary 급여, rank() over(order by salary desc) 전체순위 from employees)) where 번호 between ? and ?;
	*/
	String sql = "select 번호, 전체순위, 사원ID, 마지막이름, 급여 from(select rownum 번호, 전체순위, 사원ID, 마지막이름, 급여 from(select employee_id 사원ID, last_name 마지막이름, salary 급여, rank() over(order by salary desc) 전체순위 from employees)) where 번호 between ? and ?";
